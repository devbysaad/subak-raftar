...
export default function ComplaintsPage() {
  ...
  // Redux state
  const { list, total, pages, loading, createLoading, createSuccess, createError } =
  ...
  const [search, setSearch] = useState({
    parcelNo: '', rStatus: '', cStatus: '', fromDate: '', toDate: '', page: 1, limit: 10,
  });
  ...
  useEffect(() => {
    if (createSuccess) {
      ...
      // clear the success flag after a short delay for the banner
      const t = setTimeout(() => dispatch(resetCreateSuccess()), 4000);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    dispatch(createComplaintRequest(form));
  };

  const handleSearch = (override?: Partial<typeof search>) => {
    const merged = { ...search, ...override };
    dispatch(fetchComplaintsRequest(merged));
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    ...
                    className={inputCls}
                    value={form.parcelNo}
                    onChange={e => setForm(f => ({ ...f, parcelNo: e.target.value }))}
                  />
                </div>
                <div>
            ...
            <div className="flex flex-wrap gap-3 mb-4 items-end">
              <div className="flex-1 min-w-48">
                ...
                <input
                  ...
                  className={inputCls}
                  value={search.parcelNo}
                  onChange={e => setSearch(s => ({ ...s, parcelNo: e.target.value }))}
                />
              </div>
              <div className="w-40">
              ...
              <Button
                onClick={() => { setSearch(s => ({ ...s, page: 1 })); handleSearch({ page: 1 }); }}
                loading={loading}
                size="sm"
              >
                ...
                {pages > 1 && (
                  <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100">
                    <span className="text-xs text-gray-400">
                      Page {search.page} of {pages} · {total} complaints
                    </span>
